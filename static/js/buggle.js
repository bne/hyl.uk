...
function buggle() {
  ...
  dice.sort(() => Math.random() - 0.5).forEach((die, i) => {
    die.sort(() => Math.random() - 0.5).forEach(letter => {
...
function debuggle() {
  ...
  if (countdown == 0) {
    document.querySelectorAll('.buggle span').forEach(span => { span.innerHTML = emptyChar});
